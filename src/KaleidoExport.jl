...
const ALL_FORMATS = ["png", "jpeg", "jpg", "webp", "svg", "pdf", "eps", "json"]
...
function savefig(
    ...
    payload = Dict(
        ...
        :scale => _get(scale, 1),
        :format => replace(format, "jpg" => "jpeg"),
        :data => Plot(p)
    )
...
function savefig(io::IO,
        ...
        format::String="png")
    if format == "html"
        return show(io, MIME("text/html"), Plot(p), include_mathjax="cdn", include_plotlyjs="cdn", full_html=true)
    end
